...
{
private:
  struct qnode<ElemType> *volatile _head = NULL;  
  // 随着pop后指向的位置是不一样的, head不是固定的
...
public:
  ...
  void push2(const ElemType &e)
  {
    ...
    // 将最后的节点_tail更换为p节点
    __sync_bool_compare_and_swap(&_tail, old_t, p);  // 对比_tail是不是还是旧的，如果是那么就替换
    // compare看看是不是旧的，是旧的说明拿到了操作权限，否则就没拿到，循环继续拿
  }
  ...
  bool pop(ElemType &e)
  {
    ...
    e = np->_data;// 不为空，就取出元素，弹出

    _head->_next = np->_next;//节点后移，删除该节点
    delete np;
    return true;// 弹出是否成功
  }
...
void *queue_free_thread_push(void *argv)
{
  ...
  {
    s_queue.push2(i);
    lxx_atomic_add(&s_count_push, 1);// 统计入队次数？
...
int test_queue(thread_func_t func_push, thread_func_t func_pop, char **argv)
{
  ...
  clock_t end = clock();
  printf("spend clock : %ld, push:%d, pop:%d\n", (end - start) / CLOCKS_PER_SEC,s_count_push, s_count_pop);
  return 0;
}
